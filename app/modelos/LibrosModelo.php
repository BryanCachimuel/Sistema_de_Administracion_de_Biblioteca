...
class LibrosModelo extends Llaves
{
	...
	public function alta($data='')
	{
	   ...
	   $sql.= "'".$data['idIdioma']."', "; //2. país
	   $sql.= "'".$data['idTema']."', ";  //3. Genero
	   $sql.= "'".$data['titulo']."', ";    	//4. nombre  
	...
	public function getIntegridadReferencial($id)
	{
		//
		$ir_array = [0,0,0];
		$sql = "SELECT COUNT(*) FROM copias WHERE baja=0 AND idLibro=".$id;
		$salida = $this->db->query($sql);
		$ir_array[1] = $salida["COUNT(*)"];
		// 
		$sql = "SELECT COUNT(*) FROM librosAutores WHERE baja=0 AND idLibro=".$id;
		$salida = $this->db->query($sql);
