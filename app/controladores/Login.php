	      ...
	      if (empty($errores)) { 
			...
			if ($this->modelo->registrar($data)) {
				...
				$this->mensaje(
					...
					"Error al registrar un usuario.", 
					"Error al registrar el usuario: ".$nombres." ".$apellidoPaterno, 
					"login", 
					"danger"
				);
	        }
				...
				if ($data) {
					if ($data["clave"]==$clave) {
						...
						if($this->modelo->usuarioAutorizar($id,$clave)){
							$this->mensaje("Actualizar registro","Actualizar registro","El registro  se actualizó correctamente, bievenido(a)","login","success");
						} else {
							$this->mensaje("Error al actualizar al usuario","Error al actualizar al usuario","Error al actualizar al usuario","login","danger");
						}
					} else {
						$this->mensaje("Error al actualizar al usuario","Error al actualizar al usuario","La clave de acceso no coincide","login","danger");
					}
				} else {
					$this->mensaje("Error al actualizar al usuario","Error al actualizar al usuario","Existió un error al actualizar la clave de acceso. Favor de intentarlo más tarde o comunicarse a soporte técnico.","login","danger");
				}
				exit;
			}
		}
