...
class Libros extends Controlador
{
	...
	public function alta(){
	   //Definir los arreglos
	    ...
	    if ($_SERVER['REQUEST_METHOD']=="POST") {
	      ...
	      if($idTema=="void"){
	        array_push($errores,"El tema es obligatorio.");
	      }
	      if($idIdioma=="void"){
	        array_push($errores,"El idioma es obligatorio.");
	      }
	      if (empty($errores)) { 
	          ...
	          if ($this->modelo->alta($data)) {
	            $this->mensaje(
	          	...
	          	$this->mensaje(
	          		...
	          		"Error al añadir un libro.", 
	          		"Error al modificar libro: ".$titulo,
	          		"libros/".$pag, 
	          		"danger"
	          	);
	          }
  	...
  	public function borrar($id="",$pag=1){
    	...
    	if ($ir_array[0]==0) {
			...
			$this->mensaje(
        		...
        		"Error al borrar un libro", 
        		"No podemos eliminar el libro porque tiene:<ul><li>".$ir_array[1]." copias.</li><li>".$ir_array[2]." autores.</li></ul>Primero debe eliminar esas referencias.", 
        		"libros", 
        		"danger"
        	);
		}
	   ...
	   if (isset($id) && $id!="") {
	     if ($this->modelo->bajaLogica($id)) {
        	$this->mensaje(
        		"Borrar un libro", 
        		"Borrar un libro", 
        		"Se borró correctamente un libros.", 
        		"libros/".$pag, 
        		"success"
        	);
        } else {
	...
	* **************/
	public function librosAutores($idLibro)
	{
		...
		$libro = $this->modelo->getId($idLibro);
		$datos = [
			"titulo"=> "Autores",
	...
	public function librosAutoresAlta($idLibro=""){
  	...
  	public function librosAutoresQuitar($idLibroAutor="",$pag=1){
  		//Es el identificador de la tanla libroAutor
  		//
